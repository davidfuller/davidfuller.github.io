...
const wallaScriptingNames = ['WALLA SCRIPTED', 'WALLA SCRIPTED LINES', 'WALLA SCRIPTED LINES - SCOLDING CARRYING ON', 'WALLA SCRIPTING', 'WALLA SCRIPTING', 'WALLA SCRIPTING - lines to lead into the scripted argument', 'SCRIPTED WALLA LINES', 
                                'WALLA SCEIPTED LINES', 'SCRIPTED WALLA LINES.', 'WALLA SCRIPTED LINES.', 'WALLA SCRIPTED LINES', 'WALLA SCIPTED LINES']
...
async function parseSource(tableRowIndex = -1){
  ...
  await Excel.run(async (excel) => {
    ...
    for (let i = 1; i < theLines.length; i++){
      if ((theLines[i].trim() != '') && (theLines[i].trim().toLowerCase() != 'none') && (theLines[i].trim().toLowerCase() != 'none.') && (theLines[i].trim().toLowerCase() != 'n/a')){
