...
async function getDirectorInfo(){
  await Excel.run(async function(excel){
    ...
    for (i = 0; i < dataRange.rowCount; i++){
      ...
      if (i < myData.length){
        thisRow = [myData[i].sceneNumber, myData[i].lineNumber, myData[i].germanNumTakes, myData[i].germanMarkUp, myData[i].germanDateRecorded];
      }
  ...
  await Excel.run(async function(excel){
    ...
    if (rowIndex >= 10){
      ...
      if (!isNaN(lineNumber)){
        ...
        tempRange.select();
        await excel.sync();
        console.log('address', tempRange.address);
        await jade_modules.operations.showMainPage();
      } else {
        alert('Not a line number');
      }
