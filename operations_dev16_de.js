...
//12.01 Actor script Multiple
//13.01 Merge of V11 and V12
...
async function unlockIfLocked(){
  ...
  let isProtected = await isLocked();
  if (isProtected){
    await unlock();
  }
  return isProtected
...
async function showHideColumns(columnType){
  ...
  await Excel.run(async function(excel){ 
    ...
    if ((columnType == 'German Script') || (columnType == 'German UK Script')){
      let germanIndex = range.values.findIndex(x => x[0] == columnType);
...
async function registerExcelEvents(){
  await Excel.run(async (excel) => {
    ...
    const allSheets = excel.workbook.worksheets;
    allSheets.onActivated.add(handleSheetActivate);
...
async function clearWallaForScene(sceneNo){
  ...
  await Excel.run(async (excel) => {
    ...
    let last = -1;
    //let theIndexes = [];
    console.log('myTypecodea', myTypeCodes, 'sceneNo', sceneNo);
    for (let i = 0; i < myTypeCodes.scenes.values.length; i++){
      if (myTypeCodes.scenes.values[i] == sceneNo){
        console.log('my scene, i', i);
...
async function setSheetView(doTemporary){
  ...
  await Excel.run(async function(excel){
    ...
    await excel.sync();
    console.log('numViews', numViews.value)
    if (numViews.value > 0){
      try {
...
async function findPreviousTypeLineRowIndex(rowIndex){
  ...
  console.log('resultRowIndex', resultRowIndex);
...
async function putSceneMinMaxInSettings(){
  await Excel.run(async function(excel){
    const settingsSheet = excel.workbook.worksheets.getItem(settingsSheetName)
